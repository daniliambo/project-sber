...
import org.apache.spark.sql.catalyst.expressions.{
  ScalarSubquery => SparkScalarSubquery,
  InSubquery => SparkInSubquery,
  _
}
  ...
  def updateExpression(expression: Expression, references: ReferenceMap, aliasContext: NameContext): Expression = {
    def replaceSubquery(subquery: Expression): Subquery =
      subquery match {
        ...
        case inSubquery: SparkInSubquery =>
          val node = Converter.convertPlan(inSubquery.query.plan, references, aliasContext)
          InSubquery(inSubquery.value, node)
    ...
    expression.transform {
      ...
      case inSubquery: SparkInSubquery => replaceSubquery(inSubquery)
    ...
    expressions.traverse {
      case alias @ Alias(child, name) if name.startsWith("_w") || child.toString == name =>
        ...
        updateName(newName).map { updatedName =>
          alias.copy(name = updatedName)(alias.exprId, alias.qualifier, alias.explicitMetadata)
